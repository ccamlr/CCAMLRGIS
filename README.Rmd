...
This package relies on the [sf package](https://CRAN.R-project.org/package=sf), users may need to familiarize themselves with it.
...
plot(Bathy, breaks=Depth_cuts,col=Depth_cols,axes=FALSE,legend=FALSE)
...
#Add reference grid
add_RefGrid(bb=st_bbox(SmallBathy),ResLat=10,ResLon=20,LabLon=0,fontsize=0.75,lwd=0.75,offset = 8)
#Add color scale
...
#Plot the bathymetry
plot(B486,breaks=Depth_cuts,col=Depth_cols,legend=F,axes=F)
#Add color scale
add_Cscale(height=80,fontsize=0.7,offset=300,width=15,lwd=0.5)
...
```{r Fig09,fig.align="center",out.width="100%",message=F,dpi=200,eval=T,fig.height=3}
...
```{r Fig10,fig.align="center",out.width="100%",message=F,dpi=200,eval=T,fig.height=5}
...
plot(B486,breaks=Depth_cuts,col=Depth_cols,legend=FALSE,axes=FALSE)
#Add coastline
plot(Coast[Coast$ID=='48.6',],col='grey',lwd=0.01,add=T)
#Create pies
...
Given a bathymetry raster and a an input dataframe of latitudes/longitudes, this function computes the depths at these locations. The examples below use the 'SmallBathy' data for illustrative purposes; users should use a higher resolution bathymetry dataset instead, as obtained via the *load_Bathy()* function.
