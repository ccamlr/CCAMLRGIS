...
add_Cscale(height=90,fontsize=0.5,width=25,lwd=0.5,offset=0,maxVal=-1)
...
#Prepare layout for 2 sub-plots
par(mfrow=c(2,1))
par(mai=c(0.85,0.75,0.01,0.01)) #set plot margins as c(bottom, left, top, right)
#Plot Catch vs Depth
...
add_Cscale(title='Number of fishes',
           height=80,fontsize=0.5,width=25,lwd=0.5,offset=0
           )

#Example 2: With custom colors - here from blue to red
MyCols=add_col(MyPoints$Nfishes,cols=c('blue','red')) 
plot(MyPoints,pch=21,bg=MyCols$varcol,main='Example 2:',cex.main=0.5,cex=1.5,lwd=0.5)
box()

#Example 3: With custom colors and fewer color classes
MyCols=add_col(MyPoints$Nfishes,cols=c('blue','red'),cuts=3) 
...
add_Cscale(title='Sum of Catch (t)',cuts=Gridcol$cuts,cols=Gridcol$cols,width=33) #Add color scale using cuts and cols generated by add_col
```
