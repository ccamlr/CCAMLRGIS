...
#' 
#' @param Input \code{sf} or \code{SpatRaster (terra)} spatial object.
#' 
...
Rotate_obj=function(Input,Lon0=NULL){
  ...
  if(any(class(Input)%in%c("sf","SpatRaster"))==FALSE){
    stop("The input must be an object of class sf or SpatRaster.")
  }
